...
public class DWGraph_Algo implements dw_graph_algorithms {
    ...
    public directed_weighted_graph copy() {
        return new DWGraph_DS((DWGraph_DS) G);
    }
    ...
    public boolean isConnected() {
        if (G.nodeSize() == 1 || G.nodeSize() == 0) return true;
            ...
            if (NumOfBlack == G.nodeSize()) {
                DWGraph_DS G1 = new DWGraph_DS();
                G1 = G1.CopyOpsiteEdgeGraph((DWGraph_DS) G);
                DWGraph_Algo A1 = new DWGraph_Algo();
                A1.init(G1);
                A1.SetInit();
                ite = G1.getV().iterator();
                if (ite.hasNext()) {
                    node_data N = ite.next();
                    A1.DFS(N);
                }
                if (A1.NumOfBlack == G.nodeSize()) return true;
                else return false;
            }
            else return false;
    ...
    public double shortestPathDist(int src, int dest) {
        ...
        if (S == null || E == null) return -1;
            ...
            if(this.mc != this.G.getMC() || this.dijkstra_src != src) {
                SetInit();
                Dijkstra(S);
            }
            return E.getWeight();
    ...
    public boolean save(String file) {
        try {
            ...
            output.println(((DWGraph_DS) this.G).toJson());
            output.close();
            file_writer.close();
        }
        catch (Exception e) {
